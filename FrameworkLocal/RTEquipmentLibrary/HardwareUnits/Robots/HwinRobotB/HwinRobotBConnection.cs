...
{
    public class HwinRobotBMessage : AsciiMessage
    {
        public string Data { get; set; }
        public string ErrorText { get; set; }
    }

    public class HwinRobotBTCPConnection : TCPPortConnectionBase
    {
        private bool _isAckMode = false;//should read from config file

        public HwinRobotBTCPConnection(string address, string endof)
        ...
        protected override void ActiveHandlerProceedMessage(MessageBase msg)
        {
            lock (_lockerActiveHandler)
            {
                ...
                {
                    if (msg.IsFormatError || !((HwinRobotBHandler)_activeHandler).HasResponse || (_activeHandler.HandleMessage(msg, out bool transactionComplete) && transactionComplete))
                    {
                        _activeHandler = null;
                    }
                }
        ...
        {
            HwinRobotBMessage msg = new HwinRobotBMessage();
