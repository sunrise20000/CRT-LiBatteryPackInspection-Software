...
{
    public class HwinRobotMessage : AsciiMessage
    {
        public string Data { get; set; }
        public string ErrorText { get; set; }
    }

    public class HwinRobotTCPConnection : TCPPortConnectionBase
    {
        private bool _isAckMode = false;//should read from config file

        public HwinRobotTCPConnection(string address, string endof)
        ...
        protected override void ActiveHandlerProceedMessage(MessageBase msg)
        {
            lock (_lockerActiveHandler)
            {
                ...
                {
                    if (msg.IsFormatError || !((HwinRobotHandler)_activeHandler).HasResponse || (_activeHandler.HandleMessage(msg, out bool transactionComplete) && transactionComplete))
                    {
                        _activeHandler = null;
                    }
                }
        ...
        {
            HwinRobotMessage msg = new HwinRobotMessage();
